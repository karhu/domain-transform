...
def plot_branch_with_functions(name, data, sorted_img_list=["640", "1024",  "720p", "1080p", "2048", "4k"]):
    fig = plt.figure()
    ...
    for fn, color in zip(tested_functions, colors):
        ...
        for img in sorted_img_list:
            ...
            counter += 1
        print values
        pl = plt.bar(ind, values, width=bar_width, color=color, log=True)
        plt_bar_handles.append((pl, fn))

    plt_handles, plt_names = zip(*plt_bar_handles)
    plt.xticks(map(lambda l: l+bar_width/2.0, ind), sorted_img_list)

    plt.legend(plt_handles, plt_names, loc="best")
    plt.xlabel("Image Size")
    plt.title("Cycles/Function (log)")
    plt.grid(True)
